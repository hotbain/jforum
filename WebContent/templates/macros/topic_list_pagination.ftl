		...
		<div class="btn-group">
					<ul class="pagination">
						...
						<#if (thisPage > 1)>
							...
							<li><a href="${contextPath}/${moduleName}/${action}<#if (start > 0)>/${start}</#if><#if (id > -1)>/${id}</#if>${extension}">&#9668;</a></li>
						</#if>
						...
						<#-- 将goto部分临时注释掉
						<div ><a href="#goto" onClick="return overlay(this, 'goToBox', 'rightbottom');">${I18n.getMessage("ForumIndex.goToGo")}</a><div>
						<div id="goToBox">
...
<#macro pageLink page id>
	...
	<#if page != thisPage>
		<#assign link><li><a href="${contextPath}/${moduleName}/${action}<#if (start > 0)>/${start}</#if><#if (id > -1)>/${id}</#if>${extension}">${page}</a></li></#assign>
	<#else>
		<#assign link><li class="active"><a href="#">${page}<span class="sr-only">(current)</span></a></li></#assign>
	</#if>
